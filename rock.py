...
class Config:
    ...
    SKIP_LINE_MARKERS: tuple = (
        ("id:",) + BASE_MARKERS + ("usernames:", "имена пользователя:")
    )
...
class AsyncCache(Generic[T]):
    ...
    async def _cleanup_cache(self):
        ...
        async with self._lock:
            ...
            items = sorted(
                [(k, v, t) for k, (v, t) in self._cache.items()], key=lambda x: x[2]
            )
    ...
    async def get(self, key: Any) -> Optional[T]:
        try:
            ...
            if (
                ...
                or time.time() - self._last_cleanup >= self._cleanup_interval
            ):
        ...
        try:
            ...
            async with cls._download_locks[entity_id]:
                ...
                photo = await RetryHandler.retry_with_delay(
                    client.download_profile_photo, entity_id, bytes
                )
            ...
            logger.error(
                f"Ошибка при загрузке фото для {entity_id}: {e}", exc_info=True
            )
            return None
        finally:
            # Очищаем старые локи
            ...
            old_locks = [
                k
                for k, v in cls._last_used.items()
                if now - v > Config.CLEANUP_INTERVAL
            ]
            for k in old_locks:
...
class UserInfoMod(loader.Module):
            ...
            for attempt in range(Config.MAX_ATTEMPTS):
                try:
                    ...
                    messages = await asyncio.wait_for(
                        self._client.get_messages(chat, limit=3), timeout=Config.TIMEOUT
                    )
                    ...
                    for msg in messages:
                        ...
                        if any(
                            ...
                            for marker in Config.VALID_RESPONSE_MARKERS
                        ):
                            ...
                            for line in msg.text.split("\n"):
                                ...
                                if (
                                    ...
                                    or any(
                                        x in line.lower()
                                        for x in Config.SKIP_LINE_MARKERS
                                    )
                                    or "@" in line
                                ):
                                    continue
    ...
    async def send_info_message(
        self, message: Message, entity: Union[User, Channel], info_text: str
    ):
        try:
    ...
    async def userinfocmd(self, message: Message):
        ...
        try:
            ...
            if not args and not reply:
                await utils.answer(
                    message, "❌ Укажите пользователя/канал или ответьте на сообщение"
                )
                return
            entity = None
            try:
            ...
            try:
                ...
                if any(
                    isinstance(err, UserNotParticipantError) for err in e.exceptions
                ):
                    await utils.answer(message, "⚠️ Нет доступа к информации")
                else:
                    raise
                ...
                await utils.answer(
                    message, self.strings["error_fetching"].format(str(e))
                )
        except Exception as e:
    ...
    async def format_user_info(self, user: User, full_user: GetFullUserRequest) -> str:
        ...
        try:
            ...
            if hasattr(full_user.full_user, "phone_calls_available"):
                calls_available = full_user.full_user.phone_calls_available and getattr(
                    full_user.full_user, "video_calls_available", False
                )
            ...
            if full_user.full_user.about:
                add_section(
                    "📋 <b>О пользователе:</b>", f"<i>{full_user.full_user.about}</i>"
                )
            # Ссылки
            ...
            if funstat_info and not any(
                err in funstat_info.lower() for err in Config.ERROR_MARKERS
            ):
                add_section("📊 <b>Статистика funstat:</b>", funstat_info)
            return "\n".join(info_parts)
        except Exception as e:
            logger.error(
                f"Ошибка при форматировании информации о пользователе: {e}",
                exc_info=True,
            )
            return f"❌ Ошибка при форматировании информации: {str(e)}"

    async def format_channel_info(
        self, channel: Channel, full_channel: GetFullChannelRequest
    ) -> str:
        ...
        try:
            ...
            if full_channel.full_chat.slowmode_seconds:
                details.append(
                    f"⏱ Медленный режим: {full_channel.full_chat.slowmode_seconds} сек."
                )
            if full_channel.full_chat.linked_chat_id:
                details.append(
                    f"🔗 Связанный чат: {full_channel.full_chat.linked_chat_id}"
                )
            if (
                hasattr(full_channel.full_chat, "online_count")
                and full_channel.full_chat.online_count is not None
            ):
                details.append(f"🟢 Онлайн: {full_channel.full_chat.online_count:,}")
            if (
                hasattr(full_channel.full_chat, "messages_count")
                and full_channel.full_chat.messages_count is not None
            ):
                details.append(
                    f"💬 Сообщений: {full_channel.full_chat.messages_count:,}"
                )
            ...
            if (
                hasattr(full_channel.full_chat, "invite_link")
                and full_channel.full_chat.invite_link
            ):
                links.append(f"└ Приглашение: {full_channel.full_chat.invite_link}")
            if links:
                info_parts.extend(["", "🔗 <b>Ссылки:</b>", *links])
            # Возможности
            ...
            if features:
                info_parts.extend(["", "⚙️ <b>Возможности:</b>", " • ".join(features)])
            return "\n".join(info_parts)
        except Exception as e:
            logger.error(
                f"Ошибка при форматировании информации о канале: {e}", exc_info=True
            )
            return f"❌ Ошибка при форматировании информации: {str(e)}"
